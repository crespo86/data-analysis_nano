...
DATAFILE = '/home/shlee/Desktop/Programming/data-nano/chapter3/arachnid.csv'
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            small_data = {}
            small_data['classification'] = {}
            for a in line:
                if a in process_fields:
                    if a == "rdf-schema#label":
                        if re.search(r'(\([A-Za-z]+\))', line[a]):
                            small_data[FIELDS[a]] = re.search(r'(([A-Za-z]+))', line[a]).group()
                        else:
                            small_data[FIELDS[a]] = line[a]
                    elif a == 'name':
                        if line[a] == 'NULL' :
                            small_data[FIELDS[a]] = line['rdf-schema#label']
                        elif re.search(r'(\([A-Za-z]+\))', line[a]):
                            small_data[FIELDS[a]] = re.search(r'(([A-Za-z]+))', line[a]).group()
                        else:
                            small_data[FIELDS[a]] = line[a]
                    elif a == "synonym":
                        if line[a] == 'NULL':
                            small_data[FIELDS[a]] = None
                        else:
                            small_data[FIELDS[a]] = parse_array(str(line[a]))
                    elif FIELDS[a] in ['class', 'phylum', 'family', 'genus', 'kingdom', 'order']:
                        if line[a] == 'NULL':
                            line[a] = None
                        small_data['classification'][FIELDS[a]] = line[a]
                    else:
                        if line[a] == 'NULL':
                            line[a] = None
                        small_data[FIELDS[a]] = line[a]
